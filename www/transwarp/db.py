...
class _ConnectionCtx(object):
    '''
    _ConnectionCtx object that can open and close connection context. _ConnectionCtx can be nested and only the most outer connection has effect.
    outer 
    with connection():
        pass
        with connection():
...
def select_one(sql, *args):
    '''
    Excute select SQL and expected one result.
...
def select_int(sql, *args):
    '''
    Execute select SQL and expected one ine and only one int result.
